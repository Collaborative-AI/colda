...
def match_assistor_id():
    ...
    match_id_file_id = train_match_document['assistor_information'][user_id]['match_id_file']
    asssistor_random_id_mapping = train_match_document['asssistor_random_id_mapping']
    match_done_assistor_dict = train_match_document['match_done_assistor_dict']
    ...
    pyMongo.db.Train_Match_File.update_one({'match_id_file_id': match_id_file_id}, {'$set':{'match_id_file_content': same_id_keys}})
    log(generate_msg('2.2:', 'assistor matching', user_id), user_id, task_id)

    match_done_assistor_dict[user_id] = True
    pyMongo.db.Train_Match.update_one({'task_id': task_id}, {'$set':{'match_done_assistor_dict': match_done_assistor_dict}})
    if len(match_done_assistor_dict) == len(asssistor_random_id_mapping):
        log(generate_msg('2.3:', 'assistor matching_done', 'number of assistor:', len(match_done_assistor_dict)), user_id, task_id)
...
def match_test_assistor_id():
    ...
    match_id_file_id = test_match_document['assistor_information'][user_id]['match_id_file']
    asssistor_random_id_mapping = test_match_document['asssistor_random_id_mapping']
    match_done_assistor_dict = test_match_document['match_done_assistor_dict']
    ...
    match_done_assistor_dict[user_id] = True
    pyMongo.db.Test_Match.update_one({'test_id': test_id}, {'$set':{'match_done_assistor_dict': match_done_assistor_dict}})
    if len(match_done_assistor_dict) == len(asssistor_random_id_mapping):
        log(generate_msg('Test 2.3:', 'assistor matching_done', 'number of assistor:', len(match_done_assistor_dict)), user_id, task_id, test_id)
        for key, value in asssistor_random_id_mapping:
            # 要修改
            value.add_notification
