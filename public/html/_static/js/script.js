    ...
    var search = function ($section, $sidebarItem) {
      $section.children('.section, .function, .method').each(function () {
        if ($(this).hasClass('section')) {
          ...
          search(
            $(this),
            $sidebarItem.parent().find('[href="#' + sectionID + '"]')
          )
        } else {
          const $dt = $(this).children('dt')
          const id = $dt.attr('id')
          if (id === undefined) {
            return
          }

          let $functionsUL = $sidebarItem.siblings(
          ...
          const $li = $('<li />')
          const $a = $('<a />').css('font-size', '11.5px')
          const $upperA = $sidebarItem.parent().children('a')
          const $upperAParent = $upperA.parent()
