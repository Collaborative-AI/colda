...
def get_identifier_content(id):
    ...
    log(generate_msg('---- unread match id begins'), user_id, train_id)
    log(generate_msg('3.1:', 'get_user_match_id begins'), user_id, train_id)
    ...
    if isSponsor:
        ...
        log(generate_msg('3.2:', 'get_user_match_id done'), user_id, train_id)
    else:
...
def get_test_identifier_content(id):
    ...
    log(generate_msg('---- unread test match id begins'), user_id, train_id, test_id)
    log(generate_msg('Test 3.1:', 'get_user_test_match_id begins'), user_id, train_id, test_id)
    ...
    if isSponsor:
        ...
        log(generate_msg('Test 3.2:', 'get_user_test_match_id done'), user_id, train_id, test_id)
    else:
        sponsor_random_id_to_identifier_content_dict = {}
        assistor_id = user_id
        assistor_information = test_match_document['assistor_information']
...
def send_situation(id):
    ...
    data = request.get_json()
    if not data:
        return ValueError(input_data_err_msg(sys._getframe().f_code.co_name), 'You must post JSON data')
    
    expected_data = {
        'assistor_random_id_to_residual_dict': dict,
        'train_id': str
    }
    check_if_data_is_valid(
        data=data,
    ...
    user_id = obtain_user_id_from_token()
    user_document = mongoDB.search_user_document(
        user_id=id,
        username=None, 
        email=None, 
        key_indicator='user_id'
    )
    ...
    if cur_rounds_num == 1:
        log(generate_msg('3.3:', 'sponsor send_situation begins'), user_id, train_id)
    ...
    for assistor_random_id, residual in assistor_random_id_to_residual_dict.items():
        ...
        train_message_situation.create_train_message_situation_document(
            situation_id=situation_id, 
            train_id=train_id, 
    ...
    if cur_rounds_num == 1:
        train_message.create_train_message_document(train_id=train_id, cur_rounds_num=cur_rounds_num, situation_dict=situation_dict)
    elif cur_rounds_num > 1:
    ...
    if cur_rounds_num == 1:
        log(generate_msg('3.4:"', 'sponsor adds unread situation to assistors done'), user_id, train_id)
    elif cur_rounds_num > 1:
        log(generate_msg('5.4:"', 'sponsor adds unread situation to assistors done'), user_id, train_id)
    ...
    if cur_rounds_num == 1:
        log(generate_msg('3.5:"', 'sponsor add unread situation to sponsor done'), user_id, train_id)
        log(generate_msg('------------------------ unread situation done\n'), user_id, train_id)
    elif cur_rounds_num > 1:
        log(generate_msg('5.5:"', 'sponsor add unread situation to sponsor done'), user_id, train_id)
...
def send_test_output(id):
    ...
    log(generate_msg('Test 3.3:"', 'assistor send_test_output start'), user_id, train_id, test_id)
