...
# @main.route('/find_recipient_test', methods=['POST'])
# @token_auth.login_required
# def find_recipient_test():
...
#     recipient_id_list = []
#     recipient_id_queries = Matched.query.filter(Matched.sponsor_id == g.current_user.id, Matched.recipient_id_pair != g.current_user.id, Matched.task_id == task_id).all()
...
#         db.session.commit()
#         # send matched notification to the recipient
#         user.add_notification('unread request test', user.new_request()) 
#         db.session.commit()
#     # print(g.current_user.id, type(g.current_user.id),"2")
#     # A A
#     user = User.query.get_or_404(g.current_user.id)
