...
def match_recipient_id():
    ...
    record = Matched.query.filter(Matched.recipient_id_pair == g.current_user.id, Matched.task_id == task_id).first()
    ...
    all_cur_task_matches = Matched.query.filter(Matched.recipient_id_pair != sponsor_id, Matched.task_id == task_id).all()
    id_file_upload = 0
    for row in all_cur_task_matches:
        ...
        if int(row.recipient_id_pair) == g.current_user.id:
            ...
            Matched.query.filter(Matched.task_id == task_id, Matched.recipient_id_pair == g.current_user.id).update({"Matched_id_file": json.dumps(same_id_keys), "matched_done": 1})
            db.session.commit()
            id_file_upload += 1
        ...
        if id_file_upload == len(all_cur_task_matches):
            ...
            queries = Matched.query.filter(Matched.recipient_id_pair != sponsor_id, Matched.task_id == task_id).all()

            for row in queries:

                # update the db
                Matched.query.filter(Matched.task_id == task_id, Matched.recipient_id_pair == row.recipient_id_pair).update({"match_id_timestamp": datetime.utcnow()})
                db.session.commit()
