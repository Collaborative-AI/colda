...
def main():
    ...
    if from_id is not None:
        self_from_idx = np.genfromtxt(
            os.path.join(self_id, task_id, 'test', test_id, 'matched_idx', '{}.csv'.format(from_id)),
            delimiter=',').astype(np.int64)
        data = data[self_from_idx]
    output_path = []
    for i in range(round):
        model = load(os.path.join(self_id, task_id, 'train', str(i), 'model.pkl'))
        output = model.predict(data)
        makedir_exist_ok(os.path.join(self_id, task_id, 'test', test_id, str(i), 'output'))
        output_path_i = os.path.join(self_id, task_id, 'test', test_id, str(i), 'output', '{}.csv'.format(self_id))
